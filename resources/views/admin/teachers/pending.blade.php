{{-- resources/views/admin/teachers/pending.blade.php --}}
<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    ...
    <div class="admin-container">
        ...
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="{{ route('admin.teachers.pending') }}" class="active">👨‍🏫 Duyệt Giáo Viên</a></li>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($teachers as $teacher)
                                <tr>
                                    <td>{{ $teacher->MaNguoiDung }}</td>
                                    <td>{{ $teacher->HoTen }}</td>
                                    <td>{{ $teacher->TenDangNhap }}</td>
                                    <td>{{ $teacher->Email }}</td>
                                    <td>
                                        <span class="badge badge-pending">{{ $teacher->TeacherStatus }}</span>
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <form method="POST" action="{{ route('admin.teachers.approve', $teacher->MaNguoiDung) }}" style="display: inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-approve" onclick="return confirm('Duyệt tài khoản giáo viên này?')">✅ Duyệt</button>
                                            </form>
                                            <form method="POST" action="{{ route('admin.teachers.reject', $teacher->MaNguoiDung) }}" style="display: inline;">
                                                @csrf
                                                @method('PUT')
