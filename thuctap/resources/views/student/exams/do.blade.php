...
<div class="exam-main">
    ...
    <div class="question-card" id="q{{ $cau->MaCauHoi }}">
        ...
        <div class="question-content">
            @if($cau->HinhAnh)
                <img src="{{ asset('storage/' . $cau->HinhAnh) }}" alt="Câu hỏi {{ $i + 1 }}">
            @else
                {!! $cau->NoiDungCH !!}
            @endif
        </div>
    ...
    <div class="question-card" id="q{{ $cau->MaCauHoi }}">
        ...
        <table class="ds-table">
            ...
            <tbody>
                ...
                <tr>
                    <td><strong>{{ strtoupper($y->KyHieu) }}</strong></td>
                    <td>
                        <input type="radio"
...
<div class="q-map" id="qMap">
    ...
    <div class="q-map-grid" id="qMapGrid">
        @php $total = $phan1->count() + $phan2->count() + $phan3->count(); @endphp
        @for($n = 1; $n <= $total; $n++)
        <div class="q-dot" data-n="{{ $n }}" onclick="scrollToQ({{ $n }})">{{ $n }}</div>
        @endfor
    </div>
    <div style="margin-top: 12px; font-size: 11px; color: var(--text-muted);
                padding-top: 10px; border-top: 1px solid rgba(203,213,225,0.7);">
        Đã làm: <span style="font-weight:700; color:var(--blue);" id="doneCount">0</span>/<span>{{ $total }}</span>
    </div>
</div>
...
<form id="formNopBai" method="POST" action="{{ route('student.exams.submit') }}" style="display:none;">
    @csrf
...
<script>
    ...
    const DA_CHON_TN = @json($daDuocChonTN);
    const DA_CHON_DS = @json($daDuocChonDS);
    const DA_CHON_SO = @json($daDuocChonSo);
    const TOTAL_Q    = {{ $total ?? ($phan1->count() + $phan2->count() + $phan3->count()) }};
</script>
